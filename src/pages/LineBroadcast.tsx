  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (error.response?.status === 429) {
        errorMessage = 'LINE API Rate limit exceeded. Please try again later.';
      } else if (error.response?.status === 403) {
        errorMessage = 'You have reached your monthly LINE broadcast limit.';
      } else {
        errorMessage = error.response?.data?.message || error.response?.data?.error || error.response?.data?.details || error.message;
      }
